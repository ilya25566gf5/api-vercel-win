...
const AnalisController = {
    getExpensesSummary: async (req, res) => {
        try {
            ...
            const queryAverageLastThreeMonths = `
            SELECT AVG(amount) AS average_last_three_months
            FROM Expenses
            WHERE MONTH(date) >= MONTH(DATE_SUB(NOW(), INTERVAL 3 MONTH))
            AND YEAR(date) = YEAR(DATE_SUB(NOW(), INTERVAL 3 MONTH));
            `;
...
const [rowsAverageLastThreeMonths, fieldsAverageLastThreeMonths] = await pool.query(queryAverageLastThreeMonths);
...
if (rowsLastMonthExpenses.length > 0 && rowsMaxExpenseDescription.length > 0) {
    const averageLastThreeMonthsValue = Number(rowsAverageLastThreeMonths[0]?.average_last_three_months || 0);
